...
export default function App(){
 const tracker=useTrackerData(),[setup,setSetup]=useState(false),[route,setRoute]=useState(()=>location.hash.slice(1));
 ...
 useEffect(()=>{if(tracker.ready&&!tracker.selection)setSetup(true);},[tracker.ready,tracker.selection]);
 ...
 return <>{themes.map((style,index)=>{const id=/id="([^"]+)"/.exec(style.attributes)?.[1],kind=id?.replace('theme-',''),enabled=!kind||kind==='glass'?tracker.theme!=='poster'||!kind:tracker.theme===kind;return <style key={index} media={enabled?'all':'not all'}>{style.css}</style>;})}
  <div className="wrap"><main id="views">{current?<SubjectView key={current.id} entry={current} ratings={tracker.ratings} onRate={tracker.rate} onBack={()=>navigate(null)} attempts={tracker.attempts} onAttempt={tracker.attempt}/>:<Dashboard active={active} ratings={tracker.ratings} onOpen={navigate} onSetup={()=>setSetup(true)}/>}</main>
   <AppFooter sync={tracker.sync} status={tracker.status} theme={tracker.theme} onTheme={tracker.setTheme} onExport={tracker.exportBackup} onImportFile={tracker.importFile} onImportDesktop={tracker.importDesktop} onReset={()=>tracker.reset(current,active)} active={active} isDesktop={tracker.isDesktop}/>
  </div>
  {setup&&<Setup selection={tracker.selection} onSave={ids=>{tracker.setSelection(ids);setSetup(false);navigate(null);}} onCancel={()=>setSetup(false)}/>}</>;
}
