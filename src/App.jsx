...
export default function App(){
 ...
 return <><DesktopTitlebar/>{themes.map((style,index)=>{const id=/id="([^"]+)"/.exec(style.attributes)?.[1],kind=id?.replace('theme-',''),enabled=!kind||kind==='glass'?tracker.theme!=='poster'||!kind:tracker.theme===kind;return <style key={index} media={enabled?'all':'not all'}>{style.css}</style>;})}
  <div className={'wrap '+(tracker.ready&&!welcome&&!current?'dashboard-wrap':'')}><main id="views">{!tracker.ready?<div className="empty" role="status">Loading your revision tracker…</div>:welcome?<Welcome onContinue={()=>{setWelcome(false);setSetup(true);}}/>:current?<SubjectView key={current.id} entry={current} ratings={tracker.ratings} ratingHistory={tracker.ratingHistory} onRate={tracker.rate} onBack={()=>navigate(null)} attempts={tracker.attempts} onAttempt={tracker.attempt}/>:<Dashboard active={active} ratings={tracker.ratings} onOpen={navigate} onSetup={()=>setSetup(true)}/>}</main>
